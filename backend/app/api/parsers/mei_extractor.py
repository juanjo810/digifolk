...
class MEIExtractor:
    ...
    def xml_to_mei(self, xml_path, mei_path):
        ...
        # Load MusicXML file
        musicxml_data = open(xml_path, 'r').read()

        # Convert MusicXML to Humdrum
        humdrum_data = self.tk.parse_musicxml(musicxml_data)

        # Save Humdrum data to a file
        humdrum_file = 'output.hum'
        with open(humdrum_file, 'w') as f:
            f.write(humdrum_data)
        
        # Convert Humdrum to MEI
        self.tk.humdrumToMEI(humdrum_file, mei_path)
